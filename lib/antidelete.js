...
async function handleAntidelete(conn, updates, store, botNumber) {
    try {
        ...
        for (const update of updates) {
            ...
            let deletedMessageKey = null;

            // Path 1: delete arrives as a protocolMessage with the update payload
            if (update.update && update.update.message && update.update.message.protocolMessage
                && update.update.message.protocolMessage.type === 0) {
                deletedMessageKey = update.update.message.protocolMessage.key;
            }
            // Path 2: delete arrives as a bare REVOKE stub with no message payload
            // (messageStubType 68 = REVOKE in Baileys). This is the common case for
            // "Delete for everyone" and was previously being silently missed.
            else if (update.update && update.update.messageStubType === 68) {
                deletedMessageKey = key;
            }
